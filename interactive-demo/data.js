...
            <div class="w-56 h-56 md:w-64 md:h-64 relative">
...
        <div id="create-new-wizard-card" class="bg-gray-950/30 rounded-2xl p-8 border border-gray-700 border-dashed shadow-xl flex flex-col transform hover:-translate-y-2 transition-transform duration-300 items-center justify-center text-center hover:border-emerald-500 hover:text-emerald-400 cursor-pointer min-h-[280px]">
...
    },
    log_events: {
        rows: "21.1 M", included: true,
        desc: "Records key user interactions and events within the application. Used for funnel analysis, feature adoption tracking, and user behavior studies.",
        questions: ["What is the user adoption funnel for our new feature?", "Which are the most used features?", "Show the daily active users trend."],
        fields: [
            { name: 'event_id', type: 'string', desc: 'Unique identifier for each event log.' },
            { name: 'user_id', type: 'integer', desc: 'Identifier for the user performing the action.' },
            { name: 'event_name', type: 'string', desc: 'Name of the event being tracked.', values: [{ value: 'view_dashboard', pct: 35.0 }, { value: 'create_report', pct: 25.0 }, { value: 'share_report', pct: 15.0 }, { value: 'add_user', pct: 5.0 }] },
            { name: 'timestamp', type: 'timestamp', desc: 'Timestamp when the event occurred.' }
        ]
    }
};
...
        questionMap: {
            'arr': { sql: "SELECT\n  DATE_TRUNC('quarter', order_date) AS quarter,\n  SUM(total_amount) / 1000000 AS total_arr_millions\nFROM fct_orders\nGROUP BY 1\nORDER BY 1;", text: null, chartType: 'arr' },
            'value': { sql: "SELECT\n  AVG(total_amount)\nFROM fct_orders\nWHERE status = 'COMPLETED';", text: "The Average Order Value (AOV) for completed orders is currently <strong class='text-emerald-400'>$142.55</strong>. This is up <strong class='text-emerald-400'>3.1%</strong> from last month.", chartType: null, },
            'segments': { sql: "SELECT\n  c.industry,\n  SUM(o.total_amount) AS total_revenue\nFROM fct_orders o\nJOIN dim_customers c ON o.customer_id = c.customer_id\nGROUP BY 1\nORDER BY 2 DESC\nLIMIT 5;", text: null, chartType: 'segments' }
        }
    },
    'Growth Strategist': {
...
        questionMap: {
            'users': { sql: "SELECT\n  COUNT(customer_id)\nFROM dim_customers\nWHERE DATE_TRUNC('month', signup_date) = DATE_TRUNC('month', CURRENT_DATE);", text: "This month, <strong class='text-indigo-400'>12,450</strong> new users have signed up. We are on track to exceed last month's total of 15,100.", chartType: null, },
            'returning': { sql: "SELECT \n  CASE WHEN is_first_order THEN 'New' ELSE 'Returning' END as customer_type, \n  COUNT(DISTINCT order_id) as order_count \nFROM fct_orders \nWHERE order_date >= CURRENT_DATE - INTERVAL '4 weeks' \nGROUP BY 1;", text: "Here is the breakdown of new vs. returning orders for the last four weeks.", chartType: 'newVsReturn' },
            'channels': { sql: "SELECT \n  channel, \n  SUM(spend) / COUNT(DISTINCT new_users) as cost_per_acquisition \nFROM marketing_spend \nGROUP BY 1 \nORDER BY 2 ASC;", text: "Google Ads appears to be the most effective channel with the lowest Cost Per Acquisition.", chartType: 'segments' }
        }
    },
    'Product Analyst': {
        color: 'blue', icon: 'pie-chart',
        description: 'Tracks feature adoption, user engagement, and funnels.',
        tables: ['log_events', 'dim_customers'],
        subtitle: 'Ask questions about feature usage, user funnels, and engagement.',
        placeholder: 'Ask about DAU, feature funnels...',
        questions: ['What is the feature adoption funnel?', 'Show me Daily Active Users', 'Top 5 most frequent events?'],
        questionMap: {
            'funnel': { sql: "SELECT\n event_name,\n COUNT(DISTINCT user_id) as user_count\nFROM log_events\nWHERE event_name IN ('View Feature Page', 'Click Try It', 'Configure Feature', 'Save Configuration')\nGROUP BY 1\nORDER BY FIELD(event_name, 'View Feature Page', 'Click Try It', 'Configure Feature', 'Save Configuration');", text: "Here is the user funnel for the new feature adoption. It seems there's a significant drop-off after the initial view.", chartType: 'featureFunnel' },
            'daily active': { sql: "SELECT\n DATE_TRUNC('day', timestamp) as day,\n COUNT(DISTINCT user_id) as dau\nFROM log_events\nWHERE timestamp >= CURRENT_DATE - INTERVAL '7 days'\nGROUP BY 1 ORDER BY 1;", text: "Daily Active Users are trending upwards, with <strong class='text-blue-400'>21,400</strong> active today.", chartType: null },
            'events': { sql: "SELECT\n event_name, COUNT(*) as event_count\nFROM log_events\nGROUP BY 1\nORDER BY 2 DESC\nLIMIT 5;", text: "The most frequent user actions are related to viewing dashboards and creating reports.", chartType: 'segments' }
        }
    },
    'Ops Optimizer': {
...
        questionMap: {
            'rate': { sql: "SELECT\n  100.0 * COUNT(CASE WHEN status = 'RETURNED' THEN 1 END) / COUNT(*)\nFROM fct_orders;", text: "Our current return rate is <strong class='text-rose-400'>9.8%</strong> across all orders.", chartType: null },
            'status': { sql: "SELECT\n  status, COUNT(*) as count\nFROM fct_orders\nGROUP BY 1;", text: "Here is the distribution of all order statuses.", chartType: 'segments' },
            'ship': { sql: "SELECT\n  AVG(shipping_date - order_date) as avg_shipping_time\nFROM fct_orders\nWHERE status = 'COMPLETED';", text: "The average time from order to shipment is <strong class='text-rose-400'>1.8 days</strong>.", chartType: null, }
        }
    }
};
...
        featureFunnel: { title: 'Feature Adoption Funnel', type: 'bar', sql: WIZARD_DATA['Product Analyst'].questionMap['funnel'].sql, data: { labels: ['View Page', 'Click "Try"', 'Configure', 'Save'], datasets: [{ data: [15210, 9870, 4120, 2050], backgroundColor: ['#3b82f6', '#10b981', '#f97316', '#8b5cf6'] }] }, options: { indexAxis: 'y', responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }}, scales: { x: { ticks: { callback: v => `${(v/1000).toFixed(0)}k` }}} } }
    },
    scorecards: {
...
        dau: { title: 'Daily Active Users', value: '21.4k', trend: '+1.5%', trendColor: 'text-emerald-400', sql: "SELECT COUNT(DISTINCT user_id) FROM log_events WHERE DATE(timestamp) = CURRENT_DATE;" },
...
    { type: 'scorecard', id: 'dau' },
    { type: 'chart', id: 'arr' },
    { type: 'chart', id: 'segments' },
    { type: 'chart', id: 'featureFunnel' },
];
