...
const TABLE_DETAILS_DATA = {
    ...
    dim_customers: {
        ...
        desc: "Dimension table with customer profile information.",
         questions: ["Who are the top 10 customers by lifetime value?", "What is the distribution of customers by industry?"],
        fields: [
    ...
    marketing_spend: { 
        ...
        questions: ["What was our total ad spend last quarter?", "Which channel has the best ROAS?"], 
        fields: [
...
const WIZARD_DATA = {
    'Revenue Analyst': {
        ...
        questionMap: {
            ...
            'which are my top 5 customer segments?': {
    ...
    'Growth Strategist': {
        ...
        questionMap: {
            ...
            'analyze new vs returning orders': {
     ...
     'Ops Optimizer': {
        ...
        questionMap: {
            ...
            'what is the average time to ship?': {
...
const feedBlueprints = {
    charts: {
        arr: { title: 'Quarterly ARR Trend', type: 'line', sql: WIZARD_DATA['Revenue Analyst'].questionMap['show me quarterly arr trend'].sql, data: { labels: ["Q1 '24", "Q2 '24", "Q3 '24", "Q4 '24"], datasets: [{ data: [1.2, 1.5, 1.4, 1.8], borderColor: '#10b981', tension: 0.4, fill: true }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }}, scales: { y: { ticks: { callback: v => `$${v}M` }}}}},
        segments: { title: 'Revenue by Customer Segment', type: 'doughnut', sql: WIZARD_DATA['Revenue Analyst'].questionMap['which are my top 5 customer segments?'].sql, data: { labels: ['SaaS', 'E-commerce', 'Fintech', 'Healthcare', 'Other'], datasets: [{ data: [45, 25, 15, 10, 5], backgroundColor: ['#3b82f6', '#10b981', '#f97316', '#8b5cf6', '#64748b'], borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { boxWidth: 12 } }}}},
        newVsReturn: { title: 'New vs. Returning Orders', type: 'bar', sql: WIZARD_DATA['Growth Strategist'].questionMap['analyze new vs returning orders'].sql, data: { labels: ['W1', 'W2', 'W3', 'W4'], datasets: [ { label: 'New', data: [120, 130, 155, 140], backgroundColor: '#3b82f6' }, { label: 'Returning', data: [250, 280, 310, 290], backgroundColor: '#10b981' } ] }, options: { responsive: true, maintainAspectRatio: false, scales: { x: { stacked: true }, y: { stacked: true } }, plugins: { legend: { position: 'bottom', labels: { boxWidth: 12 } }} }}
    },
    scorecards: {
        aov: { title: 'Average Order Value', value: '$142.55', trend: '+3.1%', trendColor: 'text-emerald-400', sql: WIZARD_DATA['Revenue Analyst'].questionMap['what is our average order value?'].sql },
        newUsers: { title: 'New Users (MTD)', value: '12,450', trend: '+8.2%', trendColor: 'text-emerald-400', sql: WIZARD_DATA['Growth Strategist'].questionMap['how many new users signed up this month?'].sql },
        churnRate: { title: 'Monthly Churn Rate', value: '2.1%', trend: '-0.2%', trendColor: 'text-rose-400', sql: "SELECT ...; -- SQL for churn rate" }
    }
};
